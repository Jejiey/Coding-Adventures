...
while (run < runs) {
	Out = checkPoints(Pixels, 60);
	Pixels = Out[0];
	drawPoints(Pixels, Out[1]);
	run++;
	ctx.fillStyle = "red";
	ctx.fillRect(CanvasScale / 2, CanvasScale / 2, 1, 1);
}

function checkPoints(arr, angle) {
...
function drawPoints(arr, Lvalue) {
	for (let Xvalue = 0; Xvalue < CanvasScale; Xvalue++) {
		for (let Yvalue = 0; Yvalue < CanvasScale; Yvalue++) {
			...
			ctx.fillStyle = "hsl(100, 100%," + value + "%)";
